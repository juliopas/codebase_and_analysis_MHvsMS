...
class Elastomer(metaclass=Simulation_Object):
    ...
    def cure_elastomer(self, fold_coord=True, test_bad=False):
        ...
        if self.params['bond_type'] == "HarmonicBond":
            _, n_bonds_dict = self.random_harmonic_bonds(r_catch, bond_k, max_bonds, r_cut=0, dist=dist, std_scaling=6)
            lonely_M = []
            for id, n_bonds in n_bonds_dict.items():
                if n_bonds == 0:
                    lonely_M.append(id)
            if lonely_M:
                self.bond_to_neighbors(parts=self.sys.part.by_ids(lonely_M), n_nghb=n_bonds_if_0, bond_k=bond_k, r_cut=0, r_catch=r_catch_if_0, dist=dist, std_scaling=6)
    ...
    def random_harmonic_bonds(self, r_catch, bond_k=(0.001, 0.01), max_bonds=None, r_cut=0, dist="normal", std_scaling=6):
        """
        ...
        assert r_cut > r_catch or r_cut == 0, "r_cut must be larger than any bond lenght. (default -1)"
    ...
    def bond_to_neighbors(self, parts, n_nghb=3, bond_k=(0.001,0.01), r_catch=None, r_cut=0, dist="normal", std_scaling=6):
